...
In order to install the whole dataflow configured and used for this project, we can use the [`TwitterDataflow.xml`](https://github.com/AmauryDM/twitter-dataflow/blob/main/files/TwitterDataflow.xml) file contained in the [`files/`](https://github.com/AmauryDM/twitter-dataflow/tree/main/files) folder of the repository of the project. All that needs to be done is right click in the NiFi canva and type "Upload template" to select the file.
